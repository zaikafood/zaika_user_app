...
import 'package:zaika/features/order/widgets/dine_in_restaurants_card_widget.dart';
import 'package:zaika/features/order/widgets/track_details_view.dart';
...
class OrderTrackingScreenState extends State<OrderTrackingScreen> with WidgetsBindingObserver {
  ...
  void _loadData() async {
    // Ensure address is not null before using it
  ...
  Future<void> _loadMarkerImages() async {
    restaurantImageData = await MarkerHelper.convertAssetToBitmapDescriptor(width: 120, imagePath: Images.restaurantMarker);
    deliveryBoyImageData = await MarkerHelper.convertAssetToBitmapDescriptor(width: 120, imagePath: Images.deliveryManMarker);
    destinationImageData = await MarkerHelper.convertAssetToBitmapDescriptor(width: 120, imagePath: Images.myLocationMarker);
  }
  @override
  void initState() {
    super.initState();
    ...
    return Scaffold(
      ...
      body: GetBuilder<OrderController>(builder: (orderController) {
        ...
        if (track != null) {
          ...
          if (_controller != null && track.deliveryMan?.lat != null && track.deliveryMan?.lng != null) {
            _controller!.animateCamera(CameraUpdate.newLatLng(
              LatLng(double.parse(track.deliveryMan!.lat!), double.parse(track.deliveryMan!.lng!)),
            ));
          }
        }
        ...
        return Stack(
          children: [
            ...
            if (track != null && track.deliveryAddress?.latitude != null)
              Center(
                child: SizedBox(
                  ...
                  child: ExpandableBottomSheet(
                    background: Stack(children: [
                      ...
                      Positioned(
                        right: 15,
                        bottom: track.orderType != 'take_away' && track.deliveryMan == null ? 150 : 190,
                        child: InkWell(
                          ...
                          callback: () async {
                            ...
                            await Get.toNamed(RouteHelper.getChatRoute(
                              notificationBody: takeAway
                                  ? NotificationBodyModel(restaurantId: track.restaurant!.id, orderId: int.parse(widget.orderID!))
  ...
  void setMarker(Restaurant? restaurant, DeliveryMan? deliveryMan, AddressModel? addressModel, bool takeAway, {AddressModel? currentAddress, bool fromCurrentLocation = false}) async {
    if (restaurantImageData == null || deliveryBoyImageData == null || destinationImageData == null) {
      return;
    }
    try {
      //
      // BitmapDescriptor restaurantImageData = await MarkerHelper.convertAssetToBitmapDescriptor(width: 120, imagePath: Images.restaurantMarker);
      // BitmapDescriptor deliveryBoyImageData = await MarkerHelper.convertAssetToBitmapDescriptor(width: 120, imagePath: Images.deliveryManMarker);
      // BitmapDescriptor destinationImageData = await MarkerHelper.convertAssetToBitmapDescriptor(width: 120, imagePath: Images.myLocationMarker);

      if (fromCurrentLocation && currentAddress != null && _controller != null) {
        LatLng currentLocation = LatLng(double.parse(currentAddress.latitude!), double.parse(currentAddress.longitude!));
        _controller!.animateCamera(CameraUpdate.newCameraPosition(CameraPosition(target: currentLocation, zoom: 15)));
      }
      ...
      if (currentAddress != null) {
        tempMarkers.add(Marker(
          ...
          position: LatLng(double.parse(currentAddress.latitude!), double.parse(currentAddress.longitude!)),
          icon: destinationImageData! ,
        ));
      }

      if (addressModel != null && currentAddress == null) {
        tempMarkers.add(Marker(
      ...
      if (deliveryMan?.lat != null && deliveryMan?.lng != null) {
        tempMarkers.add(Marker(
          ...
          position: LatLng(double.parse(deliveryMan!.lat!), double.parse(deliveryMan.lng!)),
...
//                 // double.parse(track.deliveryMan!.lat!), double.parse(track.deliveryMan!.lng!),
//               ), zoom: 16),
//               minMaxZoomPreference: const MinMaxZoomPreference(0, 16),
//               zoomControlsEnabled: true,
//               markers: _markers,
...
//         icon: deliveryBoyImageData,
//       )) : const SizedBox();
//     }catch(e) {print('Error in setMarker: $e');
//    }
//
//     print('Error in setMarker: $e');
//     // print(stackTrace);
//     setState(() {});
//   }
//   //
